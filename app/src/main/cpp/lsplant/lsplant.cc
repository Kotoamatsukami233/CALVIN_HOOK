...
namespace {
// 字面量运算符：将字符串字面量的每个字符转换为 uint8_t，生成编译期字节数组，用于定义 trampoline shellcode
template <typename T, T... chars>
inline consteval auto operator""_uarr() {
    return std::array<uint8_t, sizeof...(chars)>{static_cast<uint8_t>(chars)...};
}

// 根据当前编译目标架构返回对应的 trampoline shellcode 字节数组、entry_point 偏移量（位数）和 art_method 指针偏移量（字节）
...
// 编译期确定的 trampoline shellcode 及其偏移量，在运行时通过 UpdateTrampoline 写入实际的 entry_point 偏移
auto [trampoline, entry_point_offset, art_method_offset] = GetTrampoline();

// 以下为 JNI 缓存的反射方法/字段 ID，在 InitJNI 中初始化，避免重复查找
...
// for proxy method
// 获取方法的参数类型列表和返回类型，用于动态生成代理类方法的短签名
jmethodID method_get_parameter_types = nullptr;
jmethodID method_get_return_type = nullptr;
// for old platform
// PathClassLoader 构造函数，用于在低版本 Android 上加载 DEX
jmethodID path_class_loader_init = nullptr;

// 内部使用的方法 ID 元组，hook/unhook 时需检查这些 ID 是否指向被 hook 的方法并同步更新
...
// 用户配置的生成类名、源文件名、字段名和方法名，用于动态构建 hook 所需的 DEX
...
// 验证用户传入的初始化配置信息，确保必需的名称字段不为空，并将其存储到全局变量中
...
// 缓存所有需要用到的 JNI 方法和字段 ID，包括反射 API、DexFile 构造函数、ClassLoader 等
...
// 将 ART 的 entry_point 偏移量写入 trampoline shellcode 中对应位置，以位操作方式跨字节写入
...
// 初始化所有 ART 内部组件（ArtMethod、Thread、Class、Runtime、ClassLinker、JIT 等），通过符号解析器获取 ART 内部偏移；若运行时可调试则临时关闭调试状态以绕过 ShouldUseInterpreterEntrypoint 检查
...
// 动态生成一个包含 hook 方法和 backup 方法的内存 DEX 文件，加载后返回生成的类、hooker 字段 ID 和两个方法 ID
...
// Trampoline 联合体：利用位字段在同一 uintptr_t 中同时编码页面地址和页内已分配的 trampoline 计数
// 4K 页面使用低 12 位作为计数，16K 页面使用低 14 位
union Trampoline {
public:
    uintptr_t address;
    unsigned count4k : 12;
...
// 执行实际的 hook 操作：挂起所有线程和 GC，为目标方法生成 trampoline，备份原始方法，设置入口点指向 trampoline
...
// 撤销 hook：挂起所有线程和 GC，将目标方法从备份方法恢复，保留原始访问标志
...
// 通过反射获取代理方法的参数和返回类型，构造对应的短签名（shorty）字符串（如 "IL" 表示 int 参数和对象返回值）
...
// 公开 API：初始化 LSPlant，依次执行配置验证、JNI 方法缓存和 ART 内部组件初始化，仅执行一次
...
// 公开 API：hook 一个 Java 方法，动态生成包含 hook/backup 方法的 DEX，替换目标方法的入口点为 trampoline，返回备份方法的 JNI 全局引用
...
// 公开 API：取消 hook，将目标方法从备份方法恢复，同时清理所有相关的记录
...
// 公开 API：检查指定的 Java 方法是否已被 LSPlant hook
...
// 公开 API：强制将方法设置为解释执行模式（去优化），避免 JIT 编译后的机器码影响 hook 的稳定性
...
// 公开 API：获取 native 方法的函数指针（即 ArtMethod 的 data_ 字段）
...
// 公开 API：移除类的 final 修饰符并将非 public/protected 构造函数设为可访问，使该类可被继承
...
// 公开 API：将指定的 DexFile 标记为可信，绕过某些 SELinux 或隐式 API 限制
